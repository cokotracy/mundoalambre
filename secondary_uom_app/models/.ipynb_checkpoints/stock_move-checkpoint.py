	...
	def _quantity_secondary_compute(self):
		for order in self:
			if order.product_id.is_secondary_uom:
				...
				uom_quantity = order.product_id.uom_id._compute_quantity(order.product_uom_qty, order.product_id.secondary_uom_id, rounding_method='HALF-UP') * order.product_id.weight 
		...
		for move in self:
			if move._get_move_lines():
				...
				for move_line in move._get_move_lines():
					quantity_done += move_line.secondary_uom_id._compute_quantity(move_line.secondary_done_qty, move.secondary_uom_id, round=False) * move_line.product_id.weight 
			else: 
				quantity_done = move.product_id.uom_id._compute_quantity(move.quantity_done, move.product_id.secondary_uom_id, rounding_method='HALF-UP') * move.product_id.weight 
		...
		if res:
			...
			if quantity:
				if self.product_id.is_secondary_uom:
					uom_quantity = self.product_id.uom_id._compute_quantity(quantity, self.secondary_uom_id, rounding_method='HALF-UP') * self.product_id.weight 
					uom_quantity_back_to_product_uom = self.secondary_uom_id._compute_quantity(uom_quantity, self.product_id.uom_id, rounding_method='HALF-UP') * self.product_id.weight 
	...
	def secondary_qty_compute(self):
		for move_line in self:
			if move_line.product_id.is_secondary_uom:
				...
				uom_quantity = move_line.product_id.uom_id._compute_quantity(move_line.product_uom_qty or move_line.qty_done, move_line.product_id.secondary_uom_id, rounding_method='HALF-UP') * move_line.product_id.weight 
				move_line.update({'secondary_quantity' : uom_quantity})
				uom_done_quantity = move_line.product_id.uom_id._compute_quantity(move_line.qty_done, move_line.product_id.secondary_uom_id, rounding_method='HALF-UP') * move_line.product_id.weight 
